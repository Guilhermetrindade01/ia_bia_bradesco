...
| Arquivo | Formato | Para que serve na Prada? |
|---------|---------|---------------------|
| `historico_atendimento.csv` | CSV | Contextualizar interações anteriores ou seja, dar continuidade ao atedimento de forma mais eficiente. |
...
O produto Fundo Imobiliário (FII) substituiu o Fundo Multimercado, pois pessoalmente me sinto mais confiante em usar apenas produtos financeiros que eu conheço. Assim, poderei validar as respostas da Prada de forma mais assertiva.
...
Existem duas possibilidades, injetar os dadosd diretamente no prompt (Ctrl + C, Ctrl + V) ou carregar os arquivos via código, como no exemplo abaixo:
...
transacoes = pd.hear.csv('data/transacoes.csv')
...
O exemplo de contexto montado abaixo, se baseia nos dados originais da base de conhecimento, mas os sintetiza deixando apenas as informações mais relevantes, otimizando assim o consumo de tokens. Entretanto, vale lembrar  que mais importante do que economizar tokens, é todas informações relevantes disponíveis em seu contexto.
...
- Saldo atual: R$ 10.000 (meta: R$ 15.000)
